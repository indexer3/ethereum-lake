...
func (c *ChainbaseCli) GetEventLogsByTxHashes(ctx context.Context, txHashes []common.Hash) ([]types.Log, error) {
	...
	joinComma := strings.Join(txHashesStrings, ",")

	var resp model.SQLQueryEventLog

	query := fmt.Sprintf(`select * from transaction_logs where transaction_hash in (%s)`, joinComma)
